...
\begin{tabular}{lcl}
  ...
  \definesyntax{matcher} &::=& \multilinecell{\syntax{rule} | \syntax{string} | \syntax{some-characters} | \syntax{other-characters} \\
  ...
  \definesyntax{word-char} &::=& ``\inline$\w$'' \\
...
In order for a \syntax{word-char} to \g{match}, one \g{alphanumeric} character must be found. \\

In order for \syntax{some-characters} to \g{match}, one of the \gpl{character} must be found. \\

In order for \syntax{other-characters} to \g{match}, a \g{character} that is not one of the indicated \gpl{character} must be found. \\
