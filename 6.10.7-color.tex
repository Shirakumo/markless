...
\defineidentifier{compound-color}{(color (<hex #.+>|<r ~n+> <g ~n+> <b ~n+>|<name .+>))|<name .+>}
...
\item Through an explicit colour name, contained in the \inline$name$ \g{binding}. The name must be \g{case insensitive}. The set of supported colour names is \g{implementation dependant}, though every \g{implementation} must recognise the following names: \inline$red$, \inline$green$, \inline$blue$, \inline$white$, \inline$black$.
\end{enumerate}
